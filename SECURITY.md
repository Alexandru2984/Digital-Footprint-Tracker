...
| Stolen/overprivileged API key       | Six explicit scopes, 1–365 day expiry, exact method/path policy checked against the live Vapor registry; unknown routes fail closed and account/auth/admin control-plane is session-only | `APIKeyRoutePolicy`, `APIKeyScopeMiddleware`, `AddAPIKeyAuthorization` |
...
| API key privilege creep                       | CI compares every live Vapor method/path to an exact compiled rule, rejects duplicate/unclassified routes, and denies unknown paths at runtime | `APIKeyRoutePolicy.swift`, `docs/ROUTE_AUTHORIZATION.md`                         |
