...
| Candidate fan-out / pivot amplification           | Heavy plugins (480-site sweep) run on at most one candidate (`FootprintPlugin.heavy` + `Origin.heavyEligible`); the transitive pivot is bounded to ≤2 rounds (1 for accounts, 0 for anon), light-plugins-only, ≤5 new seeds per round, each round under a 45 s deadline | `Sources/App/Services/ScanPluginRunner.swift`, `PivotExtractor.swift`, `TargetDeriver.swift`             |
